...
function xmldb_qtype_coderunner_upgrade($oldversion) {
    ...
    if ($oldversion < 2019111800) {
        ...
        // Define field tests to be added to question_coderunner_options.
        $table = new xmldb_table('question_coderunner_options');
        $field = new xmldb_field('tests', XMLDB_TYPE_TEXT, null, null, null, null, null, 'displayfeedback');

        // Conditionally launch add field tests.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
